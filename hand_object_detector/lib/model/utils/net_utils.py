...
def vis_detections_filtered_objects_PIL(im, obj_dets, hand_dets, image_path, thresh_hand=0.8, thresh_obj=0.01, font_path='lib/model/utils/times_b.ttf'):######## add image path
    ...
    if (obj_dets is not None) and (hand_dets is not None):
        ...
        for hand_idx, i in enumerate(range(np.minimum(10, hand_dets.shape[0]))):
            ...
            if score > thresh_hand:
                # viz hand by PIL
                image = draw_hand_mask(image, draw, hand_idx, bbox, score, lr, state, width, height, font, image_path) ######### add image_path
        ...
        image = vis_detections_PIL(im, 'hand', hand_dets,image_path, thresh_hand, font_path)# add  image_path
        
    return image

def vis_detections_PIL(im, class_name, dets, image_path, thresh=0.8, font_path='lib/model/utils/times_b.ttf'):# add  image_path
    """Visual debugging of detections."""
