...
import pdb
from model.utils.matching import * ##########
...
def parse_args():
  ...
  parser.add_argument('--subj', dest='subj',
                      help='load images of which subj, i.e. Dd01',
                      default="")  
  ##########################################################
...
if __name__ == '__main__':
  ...
  with torch.no_grad():
    ...
    if webcam_num >= 0 :
      ...
      if args.subj:#### subj to test exsist: look for the subj only, otherwise read the all images in the folder
          imglist=result = [i for i in imglist if i.startswith(args.subj)]
        ...
        try:#avoid broken image
            ...
            if vis and webcam_num == -1:                
                ...
                if cv2.waitKey(1) & 0xFF == ord('q'):
                    break
        except:# the image is broken
            if os.path.exists(args.image_dir+'/broken_img_'+args.subj+'_list.txt'):
                save_file_path=open(args.image_dir+'/broken_img_'+args.subj+'_list.txt','a')
                
            else:
                save_file_path=open(args.image_dir+'/broken_img_'+args.subj+'_list.txt','w')
            save_file_path.write(im_file.split('/')[-1]+'\n')
